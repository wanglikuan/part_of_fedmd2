    ...
    def __init__(self, parties, public_dataset,
        ...
        for idx, model in enumerate(parties):
            ...
            train_dataloader = DataLoader(data(total_private_data["X"], total_private_data["y"]), batch_size=batch_size,
                                          shuffle=is_shuffle,
                                          sampler=None, batch_sampler=None, num_workers=num_workers, drop_last=False)
            test_dataloader = DataLoader(data(private_test_data["X{}".format(idx//4)], private_test_data["y{}".format(idx//4)]), batch_size=batch_size,
                                         sampler=None, batch_sampler=None, num_workers=num_workers, drop_last=False)
        ...
        for i in range(self.N_parties):
            ...
            train_dataloader = DataLoader(data(private_data[i]["X"], private_data[i]["y"]), batch_size=batch_size, shuffle=is_shuffle,
                                          sampler=None, batch_sampler=None, num_workers=num_workers, drop_last=False)
            test_dataloader = DataLoader(data(private_test_data["X{}".format(i//4)], private_test_data["y{}".format(i//4)]), batch_size=batch_size,
                                         sampler=None, batch_sampler=None, num_workers=num_workers, drop_last=False)
            optimizer = Adam(model.parameters(), lr=0.001)
            criterion = nn.CrossEntropyLoss()
    ...
    def collaborative_training(self):
        ...
        while True:
            ...
            if r == self.N_rounds:                                   #adddddddddddd
                ...
                #print(len(logits))                                        #adddddddddddd
            ...
            for index, model in enumerate(self.collaborative_parties): #改了测试数据集的数据分布
                dataloader =  DataLoader(data(self.private_test_data["X{}".format(index//4)], self.private_test_data["y{}".format(index//4)]), batch_size=32,
                                        shuffle=True,
                ...
                #######写入结果######
                with open('./result/test2_baseline.txt', 'a') as f:
                    f.write('{}\t{}\t{}\n'.format(r, index, acc))
            ...
            for index, model in enumerate(self.collaborative_parties):
                ...
                train_dataloader = DataLoader(data(alignment_data["X"], logits[index]), batch_size=self.logits_matching_batchsize,  ########## SWITCH: logits<=>logits_models[index]
                                              shuffle=True,
                                              sampler=None, batch_sampler=None, num_workers=self.num_workers, drop_last=False)
